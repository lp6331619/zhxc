...
<!-- ---------------------------文本内容start----------------------------- -->
                <!-- <div class="titlebox">静态文本配置</div>
                <div
                    v-for="(child,i) in form.text_setting"
                    class="flex"
                >
                    <el-form-item
                        :label="'文本内容'"
                        :prop="`text_setting.${i}.content`"
                    >
                        <el-input
                    ...
                    <el-button
                        type="danger"
                        @click="delColor('text_setting',i)"
                    >删除</el-button>
                </div> -->
                <el-form-item label="静态文本配置">
                    <el-input
                        v-model="form.text_setting"
                        type="textarea"
                        rows="5"
                        cols="50"
                        :placeholder="`请输入文本内容`"
...
import { getVarConfigs } from '@/api/site'
export default {
    name: 'Formdian',
    props: {
        scene: {
    ...
    methods: {
        ...
        async getPrepare() {
            const varBox = await getVarConfigs({ site_id: this.siteid,partition_type:this.partition_type })
            if (varBox.code === 1) {
        ...
        addColor(e) {
            if (e === 1) {
                ...
                this.form.text_setting.push(JSON.parse(JSON.stringify(this.text_setting)))
            } else {
