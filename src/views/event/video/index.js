...
import { videoMonitorIndex, videoMonitorDel,asynAllVideos,classifyList } from '@/api/information'
...
export default {
    ...
    data() {
        return {
            ...
            searchForm: {
                ...
                class_id: this.$route.query.class_id ? JSON.parse(this.$route.query.class_id) : null,
            },
            // 搜索的列表数据类型格式
            formType: [{
                typeName: '设备监控名称',
    ...
    methods: {
        ...
        async getList() {
            ...
            const res = await videoMonitorIndex(parse)
            if (res.code === 1) {
                this.list = res.data
            }
            this.loading = false
        ...
        async getRule() {
            ...
            if (res.code === 1) {
                this.$set(this.prepare, 'class_id', res1.data.list)
            }
        },
        // 组件返回数据
        emitData(data) {
        ...
        open(row) {
            this.$router.push(`/videoDetail?id=${row.id}`)
        },
        edit(id) {
            this.$router.push(`/videoCreate?id=${id}`)
        },
        async refresh(){
          const res = await asynAllVideos()
          if (res.code === 1) {
              this.$message({
                  type: 'success',
                  message: res.msg
        ...
        // 删除
        async del(id) {
            const res = await videoMonitorDel({ id: id })
            if (res.code === 1) {
                this.$message({
                    type: 'success',
                    message: res.msg
