 ...
 * @LastEditors: Teemor
 * @LastEditTime: 2020-12-16 09:59:53
-->
<template>
...
import { EventList, gatewaySelect, instrumentSelect, varlistSelect } from '@/api/information'
...
export default {
    ...
    methods: {
        ...
        async getList() {
            ...
            const res = await EventList(parse)
            if (res.code === 1) {
                this.list = res.data
                this.loading = false
            }
        ...
        outExcel() {
            require.ensure([], () => {
                ...
                const tHeader = ['ID', '变量名', '类别', '时间', '记录值', '事件信息', '记录方式', '组名称', '设备名']
                const filterVal = ['id', 'var_alias', 'class_type', 'recordtime', 'value', 'text', 'record_type', 'group_name', 'dev_name']
