        ...
        >
            ...
            :before-remove="beforeRemove"
                ...
                >
                    ...
                    >
                        <el-option
                            v-for="(items,i) in prepare.status"
                            :key="Number(i)"
                            :label="items"
                            :value="Number(i)"
                        >
...
<script>
import { fupinEdit, fupinAdd,getSearchParams } from '@/api/village'
...
export default {
    ...
    methods: {
      async getPrepare() {
          ...
          console.log(res2.code=== 1)
          if (res2.code === 1) {
              this.prepare.status = res2.data.status
              console.log(this.prepare,'mounted')
          }
      },
      //图片处理
      // 上传前
      ...
      afterLogo(e) {
          ...
          this.form.image_url = this.form.images[0]
      },
      //删除前
      beforeRemove(index,file){
        ...
        edit() {
            ...
            for(var i=0;i<this.form.images.length;i++){
              this.files.push({url: this.imgUrl + this.form.images[i]})
            }
            console.log(this.files,'edit');
            /* this.files = [{
                ...
                if (valid) {
                    ...
                    switch (this.scene) {
                        ...
                        console.log(params,'create')
                            fupinAdd(params).then((res) => {
                                this.callback(res)
                            })
                            break
                        case 'update':
                            fupinEdit(params).then((res) => {
                                this.callback(res)
                            })
                            break
                        default:
