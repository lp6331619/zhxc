import { VideoGetList,VideoDel } from '@/api/village'
...
export default {
    ...
    methods: {
        ...
        edit(item) {
            ...
            this.$router.push(`/vvideoDetail?id=${item.id}`)
        },
        // 确认
        confirm(id) {
        ...
        // 删除
        async del(id) {
            const res = await VideoDel({ id: id })
            if (res.code === 1) {
                this.$message({
                    type: 'success',
                    message: res.msg
        ...
        add() {
            ...
            this.$router.push(`/vvideoDetail?id=`)
        },
        open(row) {
            this.$router.push(`/vvideoShow?id=${row.id}`)
        },
        emitOut(d, e, s) {
            this[d] = e
            !s ? this.getList() : ''
